...
// Trail helpers
const currentTrail: number | null =
  await tradingAPI.getCurrentTrailPercent("MSFT");
...
// Single contract
const opt: OptionContract = await tradingAPI.getOptionContract(
  "AAPL240719C00150000",
);

// Helper: all strikes on a date
const strikes: OptionContract[] = await tradingAPI.getOptionChain(
  "AAPL",
  "2024-07-19",
);
...
const acts: OptionAccountActivity[] = await tradingAPI.getOptionActivities(
  "OPEXC",
  "2024-06-30",
);
```
...
const mleg: AlpacaOrder = await tradingAPI.createMultiLegOptionOrder(
  legs,
  1,
  "limit",
  0.5,
