...
export async function fetchAccountDetails({
  ...
  if (!alpacaAccountObj && auth) {
    ...
    } as unknown as types.BrokerageAccount;
  }
  ...
  if (!alpacaAccountObj) {
    try {
      ...
      alpacaAccountObj = (await adaptic.brokerageAccount.get(
        {
          id: accountId,
        } as types.BrokerageAccount,
        apolloClient,
  ...
  const { apiKey: APIKey, apiSecret: APISecret, type } = alpacaAccountObj;
...
export async function getConfiguration(
  ...
  try {
    ...
    const { apiKey: APIKey, apiSecret: APISecret } = account;
    if (!APIKey || !APISecret) {
    ...
    const adapticUpdatePromise = adaptic.brokerageAccount.update(
      {
        ...
        portfolioProfitThresholdPercent:
          updatedConfig.portfolioProfitThresholdPercent,
        reducedPortfolioTrailPercent:
          updatedConfig.reducedPortfolioTrailPercent,

        // Position Trailing Stop Service fields
        defaultTrailingStopPercentage100:
          updatedConfig.defaultTrailingStopPercentage100,
        firstTrailReductionThreshold100:
          updatedConfig.firstTrailReductionThreshold100,
        secondTrailReductionThreshold100:
          updatedConfig.secondTrailReductionThreshold100,
        firstReducedTrailPercentage100:
          updatedConfig.firstReducedTrailPercentage100,
        secondReducedTrailPercentage100:
          updatedConfig.secondReducedTrailPercentage100,
        minimumPriceChangePercent100:
          updatedConfig.minimumPriceChangePercent100,
      } as any,
      client,
    );
...
export async function getAsset(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Failed to get asset: ${response.status} ${response.statusText} ${errorText}`,
      );
    }
