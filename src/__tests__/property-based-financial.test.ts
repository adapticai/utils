 ...
 */
function positivePriceArray(
  minLength: number = 2,
  maxLength: number = 200,
): fc.Arbitrary<number[]> {
  return fc.array(fc.double({ min: 0.01, max: 1_000_000, noNaN: true }), {
    minLength,
    maxLength,
  });
}
...
function returnsArray(
  ...
  return fc.array(fc.double({ min: -0.99, max: 10, noNaN: true }), {
    minLength,
    maxLength,
  });
}
...
describe("Property-based: Returns calculations", () => {
  ...
  it("log returns are additive: sum of log returns equals ln(last/first)", () => {
    fc.assert(
      fc.property(positivePriceArray(3, 50), (prices) => {
        const returns = calculateDailyReturns(prices);
  ...
  it("daily returns array length is at most prices.length - 1", () => {
    fc.assert(
      fc.property(positivePriceArray(1, 100), (prices) => {
        const returns = calculateDailyReturns(prices);
        expect(returns.length).toBeLessThanOrEqual(
...
describe("Property-based: Beta calculations", () => {
  it("beta of a series vs itself equals 1.0", () => {
    fc.assert(
      fc.property(returnsArray(3, 100), (returns) => {
  ...
  it("beta is linear: 2x leveraged returns produce beta near 2.0", () => {
    fc.assert(
      fc.property(returnsArray(5, 100), (benchmarkReturns) => {
        // Require meaningful variance to avoid numerical instability
        const mean =
          benchmarkReturns.reduce((s, r) => s + r, 0) / benchmarkReturns.length;
        const variance =
          benchmarkReturns.reduce((s, r) => s + (r - mean) ** 2, 0) /
      ...
      fc.property(
        ...
        (portfolioReturns, constantValue) => {
          ...
          const benchmarkReturns = Array.from(
            { length: portfolioReturns.length },
            () => rounded,
          );
          const result = calculateBetaFromReturns(
            portfolioReturns,
    ...
    fc.assert(
      fc.property(
        ...
        (benchmarkReturns, multiplier) => {
          const mean =
            benchmarkReturns.reduce((s, r) => s + r, 0) /
            benchmarkReturns.length;
          const variance =
            benchmarkReturns.reduce((s, r) => s + (r - mean) ** 2, 0) /
            benchmarkReturns.length;
...
describe("Property-based: Drawdown calculations", () => {
  it("maximum drawdown is always between 0% and 100%", () => {
    fc.assert(
      fc.property(positivePriceArray(1, 200), (equity) => {
        const result = calculateDrawdownMetrics(equity);
        const mddPct = parseFloat(
          result.maxDrawdownPercentage.replace("%", ""),
        );
  ...
  it("monotonically increasing series has 0% drawdown", () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.double({ min: 0.001, max: 10, noNaN: true }), {
          minLength: 1,
          maxLength: 100,
        }),
        (start, increments) => {
  ...
  it("current drawdown is less than or equal to maximum drawdown", () => {
    fc.assert(
      fc.property(positivePriceArray(2, 200), (equity) => {
        const result = calculateDrawdownMetrics(equity);
  ...
  it("drawdownPeriod equals troughIndex minus peakIndex", () => {
    fc.assert(
      fc.property(positivePriceArray(2, 200), (equity) => {
        const result = calculateDrawdownMetrics(equity);
        expect(result.drawdownPeriod).toBe(
          result.troughIndex - result.peakIndex,
        );
      }),
      { numRuns: 300 },
    );
...
describe("Property-based: Technical indicators", () => {
  ...
  describe("RSI properties", () => {
    ...
    it("RSI returns empty array for insufficient data", () => {
      fc.assert(
        fc.property(fc.integer({ min: 3, max: 50 }), (period) => {
