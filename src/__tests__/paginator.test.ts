...
describe("paginate - cursor-based", () => {
  ...
  it("should stop when next cursor is null", async () => {
    const fetchPage = vi.fn().mockResolvedValueOnce({ items: [1], next: null });
  ...
  it("should stop when next cursor is empty string", async () => {
    const fetchPage = vi.fn().mockResolvedValueOnce({ items: [1], next: "" });

    const config: CursorPaginationConfig<
      number,
      { items: number[]; next: string }
    > = {
      type: "cursor",
      fetchPage,
      getItems: (response) => response.items,
  ...
  it("should respect maxItems limit", async () => {
    ...
    const config: CursorPaginationConfig<
      number,
      { items: number[]; next: string | undefined }
    > = {
      type: "cursor",
      fetchPage,
      getItems: (response) => response.items,
...
describe("paginate - URL-based", () => {
  it("should iterate through all pages using URLs", async () => {
    const fetchPage = vi
      ...
      .mockResolvedValueOnce({
        items: ["a", "b"],
        nextUrl: "https://api.example.com/page2",
      })
      .mockResolvedValueOnce({
        items: ["c", "d"],
    ...
    const config: UrlPaginationConfig<
      string,
      { items: string[]; nextUrl: string | null }
    > = {
      type: "url",
      fetchPage,
  ...
  it("should stop when nextUrl is null", async () => {
    ...
    const config: UrlPaginationConfig<
      number,
      { items: number[]; nextUrl: string | null }
    > = {
      type: "url",
      fetchPage,
...
describe("paginate - offset-based", () => {
  it("should iterate through all pages using offset", async () => {
    ...
    const fetchPage = vi
      .fn()
      .mockImplementation(async (offset: number, limit: number) => ({
        items: allItems.slice(offset, offset + limit),
        total: allItems.length,
      }));

    const config: OffsetPaginationConfig<
      number,
...
describe("paginate - async iterator behavior", () => {
  ...
  it("should support early break from iteration", async () => {
    ...
    const fetchPage = vi.fn().mockImplementation(async () => {
      fetchCount++;
      return {
        items: [fetchCount * 10 + 1, fetchCount * 10 + 2],
        next: `p${fetchCount + 1}`,
      };
    });
...
describe("paginateAll", () => {
  it("should collect all items into an array", async () => {
    ...
    const config: CursorPaginationConfig<
      string,
      { data: string[]; cursor: string | null }
    > = {
      type: "cursor",
      fetchPage,
      getItems: (r) => r.data,
  ...
  it("should return empty array for empty result set", async () => {
    const fetchPage = vi.fn().mockResolvedValueOnce({ data: [], cursor: null });
...
describe("Alpaca pagination patterns", () => {
  it("should support Alpaca news response pagination pattern", async () => {
    ...
    const page1: AlpacaNewsPage = {
      news: [
        { id: 1, headline: "News 1" },
        { id: 2, headline: "News 2" },
      ],
