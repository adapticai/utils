...
describe("StampedeProtectedCache", () => {
  ...
  describe("error handling", () => {
    it("should throw error when loader fails", async () => {
      ...
      await expect(cache.get("test-key", failingLoader)).rejects.toThrow(
        "Load failed",
      );
    ...
    it("should not cache failed loads", async () => {
      let callCount = 0;
      const intermittentLoader: CacheLoader<string> = vi.fn(
        async (key: string) => {
          callCount++;
          if (callCount === 1) {
      ...
      await expect(cache.get("test-key", intermittentLoader)).rejects.toThrow(
        "First call failed",
      );
