...
const OrderTypeEnum = z.enum(['market', 'limit', 'stop', 'stop_limit', 'trailing_stop']);
...
export const ExecutionPrefsObjectSchema = z.object({
  ...
  partialFillPolicy: z.enum(['accept_partial', 'cancel_remainder', 'replace_to_fill']).default('accept_partial'),
  sizingMethod: z.enum(['notional', 'quantity']).default('notional'),
  lotRoundingBehavior: z.enum(['round_down', 'round_nearest', 'round_up']).default('round_down'),
  afterHoursExecutionBehavior: z.enum(['limit_only', 'no_execution', 'normal']).default('limit_only'),
