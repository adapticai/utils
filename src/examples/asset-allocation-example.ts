...
export async function conservativeRetirementPortfolio() {
  ...
  console.log(
    `  Expected Return: ${recommendation.portfolioMetrics.expectedReturn.toFixed(2)}%`,
  );
  console.log(
    `  Expected Volatility: ${recommendation.portfolioMetrics.expectedVolatility.toFixed(2)}%`,
  );
  console.log(
    `  Sharpe Ratio: ${recommendation.portfolioMetrics.sharpeRatio.toFixed(2)}`,
  );
  console.log(
    `  Max Drawdown: ${recommendation.portfolioMetrics.maxDrawdown.toFixed(2)}%`,
  );
...
export async function aggressiveGrowthPortfolio() {
  ...
  const assetCharacteristics: AssetClassCharacteristics[] =
    getStandardAssetCharacteristics();
...
export async function rebalancePortfolio() {
  ...
  const totalValue = Array.from(currentPositions.values()).reduce(
    (sum, val) => sum + val,
    0,
  );
  ...
  currentPositions.forEach((amount, asset) => {
    ...
    console.log(
      `  ${asset}: $${amount.toFixed(2)} (${percentage.toFixed(1)}%)`,
    );
  });
  ...
  recommendation.allocations.forEach((alloc) => {
    console.log(
      `  ${alloc.assetClass}: $${alloc.amount.toFixed(2)} (${(alloc.allocation * 100).toFixed(1)}%)`,
    );
  });
  ...
  if (recommendation.rebalancing && recommendation.rebalancing.length > 0) {
    ...
    recommendation.rebalancing.forEach((action) => {
      const drift =
        Math.abs(action.currentAllocation - action.targetAllocation) * 100;
      console.log(`  ${action.assetClass}:`);
      console.log(`    Action: ${action.action}`);
      console.log(
        `    Current: ${(action.currentAllocation * 100).toFixed(1)}%`,
      );
...
export async function riskParityPortfolio() {
  ...
  console.log("\nRisk Decomposition:");
  recommendation.riskAnalysis.riskDecomposition.forEach(
    (contribution, asset) => {
...
function printRecommendation(recommendation: AllocationRecommendation): void {
  ...
  console.log(
    `  Sortino Ratio: ${recommendation.portfolioMetrics.sortinoRatio.toFixed(2)}`,
  );
  console.log(
    `  Max Drawdown: ${recommendation.portfolioMetrics.maxDrawdown.toFixed(2)}%`,
  );
  console.log(
    `  VaR (95%): ${recommendation.portfolioMetrics.valueAtRisk95.toFixed(2)}%`,
  );
  console.log(`  Risk Level: ${recommendation.riskAnalysis.riskLevel}`);
  ...
  recommendation.allocations.forEach((alloc) => {
    ...
    console.log(`    Amount: $${alloc.amount.toFixed(2)}`);
    console.log(
      `    Risk Contribution: ${(alloc.riskContribution * 100).toFixed(1)}%`,
    );
    console.log(
      `    Return Contribution: ${alloc.returnContribution.toFixed(2)}%`,
    );
  });

  console.log("\nDiversification Metrics:");
  console.log(
    `  Diversification Ratio: ${recommendation.diversification.diversificationRatio.toFixed(2)}`,
  );
  console.log(
    `  Effective Assets: ${recommendation.diversification.effectiveNumberOfAssets.toFixed(1)}`,
  );
  console.log(
    `  Average Correlation: ${recommendation.diversification.averageCorrelation.toFixed(2)}`,
  );
  console.log(
    `  Asset Class Diversity: ${recommendation.diversification.assetClassDiversity.toFixed(1)}%`,
  );

  if (recommendation.warnings.length > 0) {
    console.log("\nWarnings:");
    recommendation.warnings.forEach((warning: string) =>
      console.log(`  - ${warning}`),
    );
  }
}
